...
-- Structure de la table `enseignant_`
--
...
ALTER TABLE `enseignant_groupe`
  ADD PRIMARY KEY (`id_prof`,`classe` );
...
CREATE TABLE `absence_etudiant` (
  ...
  `nb_abs_J` int(8) DEFAULT 0 ,
  `nb_abs_NJ` int(8) DEFAULT 0 ,
  `matiere` varchar(40) NOT NULL DEFAULT '',
  `date_ab` date 
) ENGINE=InnoDB DEFAULT CHARSET=latin1;


ALTER TABLE `absence_etudiant`
  ADD PRIMARY KEY (`id_etudiant`,`date_ab`);

ALTER TABLE `absence_etudiant`
  add FOREIGN KEY (`id_etudiant`)
  REFERENCES `etudiant` (`cin`) 
  ON UPDATE CASCADE;
